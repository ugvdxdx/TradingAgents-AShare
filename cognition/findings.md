...
## 七、异动信号 → chain_tiers 的链路与两个修复 (2026-06-26)

### ✅ 链路确认: 上涨异动股是"赛道级"信号, 非"个股级"

- 上涨异动股(r20≥25%)经归因后, 通过 `sector_tag`(细分赛道)+ `summary`(涨因)两个字段喂给 `chain_tiers._gather_research_signals` → 进 `price_confirmed_hot` → 注入 `build_candidate_tier_map` 的 LLM prompt → LLM 把对应**赛道**(非个股)在 6 档里归档。
- **两条平行的价格验证链路**(互补): ① `price_confirmed_hot/cold`(归因, 含涨因); ② `sector_price_momentum`(纯量价, 各板块 r20 中位数)。prompt 规则 3.5/3.6 明确"价格 > 研报 bullish count"。
- **涨跌不对称**: 上涨/下跌都喂 chain_tiers(感知升降温), 但只有**上涨**触发 fundamentals 刷新(下跌股纯做板块风险信号)。

### ⚠ 修复1: 归因 prompt bug (选项被推理模型复述)

- **根因**: `ATTR_PROMPT_UNIFIED` 原把选项塞进模板行 `REASON_TYPE|板块供需 或 个股事件 或...`, GLM-5.2 推理模型常当"已填好的示例"原样复述 → 17 只空壳(summary 空)+ 4 只低质量(reason_type 含"或")。
- **叠加因素**: 归因用 `_llm_quick`(不重试, max_tokens=300 推理不够), 偶发空响应。
- **修复**: ① 选项移出模板行进说明、模板行用"这里写..."纯文字占位(去掉会被复述的 `<...>`)、"第一行就是答案禁止分析"强约束; ② 归因调用换 `_llm`(带 429 重试, max_tokens=1500)。
- **结果**: 159/159 归因全有效(空壳 0、低质量 0)。
- **教训**: 给推理模型写 prompt, **示例占位符(`<...>`、选项列表)** 会被当答案复述; 占位符要用"这里写..."这类纯指令文字, 并显式禁止复述。

### ✅ 修复2: chain_tiers 异动信号截断(信息损失)

- **问题**: `price_confirmed_hot[:20]` / `cold[:10]` 截断前**没按信号强度排序**, 是按 JSON 字典序随机砍。实测 76 涨 + 26 跌, 被砍掉 56 涨 + 16 跌(74%/62%), 包括涨幅最大的三孚股份(+64%)、聚石化学(+59%)。
- **为何不用调 max_tokens**: 异动信号是**输入**(进 prompt, 全量 ~3000 token, 占 128K 上下文 <3%); max_tokens 管**输出**(tier_map 是固定 6 档赛道结构, 全量异动不增大输出, 4096 够用)。
- **修复**: 删除两行截断, 全量进 prompt。
- **改动效果**(同基准 tier_map, 仅异动段输入不同的 A/B 对比):
  - **实质增益(信号驱动)**: 被砍大涨股所在赛道进入相应热度档 —— 高纯四氯化硅(三孚+64%)、先进封装电镀/通孔填充(聚石化学+59%)、PCB填孔药水(光华+55%)、石英晶振(泰晶科+41%)。
  - **下跌信号更全**: 退潮带(0-2.5)新增"预调酒/考公培训/航空货运(特定风险)"等。
  - **赛道描述更扎实**: 全量版各档普遍带涨跌依据括注(如"半导体靶材(海外转单+周期见底)"), 因 LLM 看到更多股的具体涨因。
  - **诚实补充**: 单次对比无法 100% 剥离 LLM 表述随机性(同赛道换说法属抖动, 非改动功劳); 但"被砍股所在赛道现在进档"是确定的(截断版 prompt 物理上不存在这些股信息)。
- **教训**: 截断必须按信号强度排序(如 r20 降序), 否则等于随机抽样; 且要评估截断阈值 vs 实际数据量 —— 本例 76+26 条 / 6000 字符根本没到需要截断的程度。
