...
1. **fundamentals JSON** — `fundamentals/{code}.json`，通过 `_gen_top500_fundamentals.py` 生成（见 fundamentals-generator skill）
...
| `run_backtest.py` | 全量回测（V3 vs 涨幅 Spearman 相关性） |
| `.fundamental_v3_scores.json` | ★ V3 评分缓存（544只，含 essence），唯一权威缓存 |
| `_gen_top500_fundamentals.py` | 个股基本面 JSON 生成器（见 fundamentals-generator skill） |
