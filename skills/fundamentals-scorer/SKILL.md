...
| V3 业绩兑现度(delivery, 已废弃) vs 涨幅 | ρ = 0.378 (历史; 2026-06-26 替换为 surge 爆发分) |
| 五等分 Q1→Q5 涨幅 | **+2.4% → +6.1% → +39.4% → +78.2% → +142.8%** |
| 多空收益差 | **+154.1%** |

核心洞察：赛道动量是唯一有效的选股信号。V3 三子维度（chain + surge + capital）；delivery(业绩兑现) 2026-06-26 替换为 surge(爆发分, 30天超额收益概率=成长性加速拐点×催化近度)。下方三维度 ρ 均>0.37 的回测为旧 delivery 历史值, surge 预测力待重评后回测。
...
# 产出: data/caches/fundamental_v3_scores.json (评分+essence精华)
uv run python3 picker/scoring/v3_full_score.py

# 全量回测（V3 vs 涨幅相关性）
uv run python3 picker/backtest/picker/backtest/run_backtest.py
```
...
| 维度 | 范围 | 评分锚点 |
|:---|:---:|:---|
| chain 赛道热度×竞争力 | 0.0-10.0 | 6档可重叠热度带: 最热主线(1.6T光模块/HBM/CoWoS/AI主芯片)→8.5-10；热门→7-9；温热新兴→5.5-7.5；中性→3.5-5.5；偏冷/退潮(锂电/白酒/地产)→0-4 |
| surge 爆发分 | 0.0-10.0 | 30天超额收益概率=成长性加速拐点×催化近度: 加速主升→8-10；温和加速→5.5-7.9；平稳钝化→3-5.4；失速→0-2.9。2026-06-26 替换原 delivery(业绩兑现度) |
| capital 资金关注度 | 0.0-5.0 | AI 算力主线→4-5；国产算力/半导体设备/机器人→2.5-3.9；消费电子/汽车电子→1.5-2.4；冷门→0-1.4 |

**sector_score = chain + surge + capital**（程序自动计算，不信任 LLM 的加法）

## essence 精华信息（6 字段，服务下游30天辩论）

每次 V3 打分同步产出，为 `picker/pipeline/debate_picker_v5.py` 提供弹药：
...
| 文件 | 用途 |
|:---|:---|
| `picker/scoring/fundamental_scorer.py` | V3 评分引擎（Prompt + LLM 调用 + JSON 解析 + 规则引擎 fallback） |
