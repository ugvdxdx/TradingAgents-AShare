...
# ═══════════════════════════════════════════════════════════
# Web Search (MCP web_search_prime, 复用 GLM Coding Plan 额度)
# ═══════════════════════════════════════════════════════════

# 智谱 MCP web_search_prime 端点 (Remote MCP Server)
# 与旧的 web-search-pro /tools 接口不同: 这条路计入 GLM Coding Plan 套餐额度,
# 不走易限流的 web-search-pro 独立资源包。协议为 MCP streamable HTTP (有状态会话)。
...
def refresh_from_research(days: int = 3, dry_run: bool = False,
    ...
    # 异动股也加入刷新列表 (异动结论需回流fundamentals, 不只靠研报触发)
    try:
        ...
        if os.path.exists(surge_cache_path):
            ...
            for code, entry in sc.items():
                ...
                try:
                    ...
                    pass
            if surge_added:
                print(f"异动触发额外刷新: +{surge_added} 只 (异动缓存内, 非研报提及)")
    except Exception:
        pass

    if not stocks:
        print(f"近 {days} 天无研报提及个股，也无异动触发，跳过")
        return {'updated': 0, 'failed': 0, 'stocks': []}

    print(f"刷新列表: {len(stocks)} 只 (研报提及 + 异动触发)")
    if dry_run:
