...
LLM_CACHE_FILE = paths.FUNDAMENTAL_LLM_SCORES_PATH

# ============ 可配置：当前市场主线 + 回测窗口 ============
# 市场主线会轮动，更新此处即可让两套 Prompt 同步跟随，无需改 Prompt 正文。
# 也可由研报知识系统的板块资金流动态注入（覆盖 MARKET_THESIS_ASOF/MARKET_THESIS_BLOCK）。
MARKET_THESIS_ASOF = "2026-06"
MARKET_THESIS_BLOCK = """主线：光模块/CPO/PCB/先进封装/存储/HBM/AI芯片 → 业绩正在加速兑现，资金持续流入
- 副线：消费电子复苏+AI端侧落地、AI电源/液冷/铜连接 → 有实际订单支撑
- 退潮线：锂电/白酒/地产/传统矿业 → 旧赛道资金流出，即使基本面不差
- 冷门线：银行/电力/公用事业 → 无催化，随大盘"""

# 回测验证窗口（滚动更新时同步修改，避免结论失真）
BACKTEST_WINDOW = "2025.12-2026.06"
BACKTEST_N = 539
...
## 回测验证结论
对__BACKTEST_N__只A股__BACKTEST_WINDOW__的实证回测表明：
...
（注：以上结论基于 __BACKTEST_WINDOW__ 窗口，窗口滚动后需重新验证）
...
评估公司是否会暴雷，而非能否涨。综合参考 weaknesses、growth_assessment、financial_metrics 等字段：

1. 是否有硬伤？(0-10分)：利润为负 = 0分；ROE<3% → 上限4分；净利率<5%且营收>千亿 → 上限5分。反之ROE>20%且净利率>20% → 8-10分
2. 护城河 (0-8分)：真龙头有定价权 → 6-8分；细分龙头 → 3-5分；无优势 → 0-2分。参考 strengths 和 weaknesses 综合判断
3. 财务安全 (0-7分)：低负债+现金流好 → 5-7分；合理 → 2-4分；危险 → 0-1分。参考 weaknesses 中的财务风险提示

**关键红线**：基本面<10分的股票应被排除，不参与赛道排名。

### 第二部分：赛道动量（0-25分）— Alpha信号
这是唯一有效的选股信号，需要最精细的判断。综合参考 what_they_do、strengths、weaknesses、growth_assessment、geopolitical_assessment、summary、research_catalysts 全部字段：

1. 产业链位置 (0-10分)：从 what_they_do 和 strengths 判断真实业务，不要只看 industry 标签。同时参考 growth_assessment.growth_drivers 确认增长逻辑是否与产业链位置一致，参考 weaknesses 排除虚假定位
   - AI算力核心供应商（光模块/PCB/先进封装/AI芯片/存储/HBM）→ 7-10分
   - 间接配套/消费电子/汽车电子 → 4-6分
   - 传统行业主业，但有研报背书的高动量新增长极（见下方 research_catalysts 规则）→ 3-6分
   - 产业链外但独立成长逻辑清晰 → 1-3分
   - 传统行业无催化/旧赛道退潮（锂电/白酒/地产/矿业）→ 0分
2. 业绩兑现度 (0-10分)：参考 growth_assessment.growth_drivers（真实催化）和 growth_assessment.headwinds（增长逆风），以及 weaknesses 中的经营风险
   - 有明确大客户（英伟达/华为/苹果/特斯拉等）+ 产能扩张+业绩高增 → 7-10分
   - 有客户但未放量/业绩增速一般 → 3-6分
   - 只有概念无订单/"国产替代/一带一路/政策红利"等空话 → 0-2分
3. 资金关注度 (0-5分)：参考 geopolitical_assessment.opportunities（政策/地缘催化）、summary 中的研报近期观点、以及 research_catalysts 的研报背书
   - AI算力/光模块/先进封装/存储 → 4-5分
   - 消费电子复苏/汽车电子、或传统行业有研报背书的高动量催化 → 2-3分
   - 冷门/资金流出 → 0-1分

### research_catalysts 使用规则（关键，解决"传统行业误伤"）
research_catalysts 是从财经博主研报中提取的**外部市场视角**催化（与公司自述隔离，可信度更高）：
- high_momentum_exposure（0-5）：研报背书的高动量赛道暴露度
- catalyst_tags：命中的高动量赛道标签（如"AI数据中心用铜""战略金属""PCB钻针"）
判断逻辑：
- 若主业属传统/旧赛道，但 high_momentum_exposure ≥ 3，说明有研报反复背书的真实新增长极 → 产业链位置给 3-6分（而非0），资金关注度给 2-3分。给分高低取决于：催化确定性、研报新近度、新增长极的营收占比（占比小则取下限）
- high_momentum_exposure 为 1-2 → 视为题材扰动，产业链位置最多 +1~2分修正，不突破中间档
- high_momentum_exposure 为 0 或字段为空 → 完全按主业判断，旧赛道诚实给0分
- 注意：research_catalysts 只能"托底加分"，不能让真旧赛道股冒充AI核心标的；上限是中间档（6分）

注意：赛道分需要拉开区分度。全给10分或全给0分都没意义。光模块核心供应商给8-10分，PCB给6-8分，半导体设备给5-7分，边缘配套给2-4分。

非AI链优质公司（创新药、高端消费）如果独立成长逻辑清晰，产业链给1-3分，资金关注给1-2分。
旧赛道退潮品种（锂电/白酒/地产/传统矿业）若无研报背书的新增长极，产业链和资金关注度诚实给0分。
...
def _fill_prompt_tokens(prompt: str) -> str:
    """注入可配置的市场主线 / 回测窗口占位符。

    支持环境变量覆盖（便于研报知识系统动态注入当前主线）:
      TA_MARKET_THESIS_ASOF / TA_MARKET_THESIS_BLOCK
    """
    asof = os.environ.get("TA_MARKET_THESIS_ASOF") or MARKET_THESIS_ASOF
    thesis = os.environ.get("TA_MARKET_THESIS_BLOCK") or MARKET_THESIS_BLOCK
    return (prompt
            .replace("__ASOF__", asof)
            .replace("__MARKET_THESIS__", thesis)
            .replace("__BACKTEST_WINDOW__", BACKTEST_WINDOW)
            .replace("__BACKTEST_N__", str(BACKTEST_N)))


def _call_llm_score(data_json: str) -> Optional[dict]:
    """[V1] 调用 LLM 打分，返回 {"score": int, "brief": str} 或 None"""
    content = _call_llm_api(_fill_prompt_tokens(SCORING_PROMPT) + data_json[:8000])
    if not content:
        return None
...
def _call_llm_score_v2(data_json: str) -> Optional[dict]:
    ...
    content = _call_llm_api(_fill_prompt_tokens(SCORING_PROMPT_V2) + data_json[:8000])
    if not content:
        return None
    result = _parse_llm_response(content)
