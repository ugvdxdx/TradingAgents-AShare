...
KLINE_DIR = os.path.join(SCRIPT_DIR, "kline_cache")
V3_CACHE = os.path.join(SCRIPT_DIR, ".fundamental_v3_scores.json")
# 归因缓存: 搜索过的上涨原因记录于此, 避免每日重复搜索
ATTR_CACHE = os.path.join(SCRIPT_DIR, ".mispriced_attribution_cache.json")
...
def _reactivate_cold_stocks():
    ...
    """
    cold_dir = os.path.join(SCRIPT_DIR, "cold_fundamentals")
    cold_list_path = os.path.join(SCRIPT_DIR, ".cold_stocks.json")
...
def _update_sub_sector_override(gems: list, meta: dict):
    ...
    """
    override_path = os.path.join(SCRIPT_DIR, ".sub_sector_override.json")
    # 加载现有 (从默认值开始)
    try:
        from _v3_full_score import _SUB_SECTOR_OVERRIDE_DEFAULT
    ...
    v3_path = os.path.join(SCRIPT_DIR, ".fundamental_v3_scores.json")
