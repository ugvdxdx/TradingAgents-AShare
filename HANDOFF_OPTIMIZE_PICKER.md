...
> ⚠️ **2026-06-26 更新**: 本文档中 `delivery`(业绩兑现) 已替换为 **surge 爆发分**(30天超额收益概率=成长性加速拐点×催化近度), 锚公式 `chain+capital×2-delivery×0.5` 改为 `chain+capital×2+surge×SURGE_WEIGHT`(+1.0, 待回测)。delivery 全池 ρ=+0.082 正向(原 −0.5 负权重是基于新晋股子池 −0.33 的错误外推)。下方 delivery 相关内容为历史参考, 字段已全量重命名为 surge。
